<?php
    $query_result=$obj_admin->select_all_sale_customer();
    if(isset($_GET['status']))
    {
        ...
        if($status=='active')
         {
             $obj_admin->active_sale($id);
         }
        else if($status=='inactive')
         {
             $obj_admin->inactive_sale($id);
         }
        else if($status=='delete')
        {
            $obj_admin->delete_sale($id);
        }                
    }
    $page_one=0;
    if (isset($_GET['pages'])) {
    ...
    $sql="SELECT * FROM tbl_sales_info WHERE deletion_status='0' LIMIT $page_one,25";
...
<div class="common_wrap">
    ...
    <div class="manage_wrap">
        <div class="row">
            ...
            <div class="purchse_cell_1 bg_color_1 fontsize_12">S Id</div>
            <div class="cell_1 bg_color_1 fontsize_12">S Invoice</div>
            ...
            <div class="cell_1 bg_color_1 fontsize_12">Discount (Tk.)</div>
        ...
        <div class="row">
            ...
            <div class="cell_1 color_1"><?php echo $row['s_invoice']?></div>
            ...
            <div class="cell_1 color_4"><?php echo  number_format((float)$row['discount_amnt'], 2, '.')?></div>
            <div class="cell_1 color_1"><?php echo  number_format((float)$row['total_customer_paid'], 2, '.')?></div>          
            <?php
                if($row['status']==1)
                {
            ?>
            ...
            <div class="cell_1 color_1 manage_link_btn">
                <?php
                    ...
                    {
                ?>
                <a class="btn_active" href="?status=active&id=<?php echo $row['sales_id']?>" title="Inactive"></a>
                <?php } else{?>
                <a class="btn_inactive" href="?status=inactive&id=<?php echo $row['sales_id']?>" title="Active"></a>
                <?php } ?>
                <a class="btn_edit" href="edit_sale.php?id=<?php echo $row['sales_id']?>" title="Edit"></a>
                <a class="btn_delete" href="?status=delete&id=<?php echo $row['sales_id']?>" onclick="return check_delete();" title="Delete"></a> 
            </div>
        </div>        
        <?php $cnt=$cnt+1; }?>
    </div>
